...
const resend = process.env.RESEND_API_KEY
  ...
  : null;
const from =
  process.env.RESEND_FROM || "NexusNova Studio <onboarding@resend.dev>";
  ...
  if (notify)
    tasks.push(
      resend.emails.send({
        from,
        to: notify,
        subject: `New Lead: ${input.name} - ${input.service_needed}`,
        html: adminHtml,
      }),
    );
  tasks.push(
    resend.emails.send({
      from,
      to: input.email,
      subject: "We received your NexusNova Studio inquiry",
      html: clientHtml,
    }),
  );
