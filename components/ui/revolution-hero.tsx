...
function NavLink({ children, href, gradient }: NavLinkProps) {
  ...
  return (
    <Link
      ref={linkRef}
      href={href}
      className={`block mb-2 text-4xl md:text-6xl lg:text-8xl font-black leading-tight cursor-pointer transition-all duration-300 transform-gpu [perspective:1000px] ${
        isHovered ? "z-10" : ""
      }`}
      style={{
        background: gradient,
...
const navLinks = [
  { text: "TRIAGE", href: "/solutions", gradient: "linear-gradient(135deg, #ffffff, #cccccc)" },
  { text: "AUTOMATE", href: "/solutions", gradient: "linear-gradient(135deg, #ffffff, #cccccc)" },
  { text: "RETAIN", href: "/solutions", gradient: "linear-gradient(135deg, #ffffff, #cccccc)" },
  { text: "DEPLOY", href: "/contact", gradient: "linear-gradient(135deg, #ffffff, #cccccc)" },
]
...
export default function WebGLHero() {
  ...
  const intensityRef = useRef({ value: 1.0 })
  ...
  const initGL = () => {
    ...
    const handleMouseMove = (e: MouseEvent) => {
      ...
      gsap.killTweensOf(intensityRef.current)
      gsap.to(intensityRef.current, {
        value: 1.15,
        duration: 0.3,
        ease: "power2.out",
        onUpdate: function () {
          intensityRef.current.value = this.targets()[0].value
        },
      })
  ...
  useEffect(() => {
    ...
    const animateFrame = () => {
      ...
      if (gl && program && buffer && timeLocation && resolutionLocation && mouseLocation && intensityLocation) {
        ...
        gl.uniform1f(intensityLocation, intensityRef.current.value)
  ...
  return (
    <section className="relative h-screen w-full overflow-hidden bg-black">
      ...
      <div className="relative z-10 h-full flex flex-col justify-between p-8 pt-32 md:p-12 md:pt-40">
        {/* Top text */}
